...
trait Printable[T] { self: T =>
  ...
  def toDocument: Document = {
    val printedFields: Traversable[(String, Either[String,Document])] = (getMappable.toMap(self) map { case (key, value) =>
      value match {
        case Some(obj) => Some((key,Printable.toEither(obj)))
        case None => None
        case obj => Some((key,Printable.toEither(obj)))
      }
    }).flatten.toTraversable
...
object Printable {
  ...
  private def toEither[T](obj: T): Either[String, Document] = obj match {
    ...
    case value: Traversable[_] => Right(nest(4, getDocumentForTraversable(value)))
    case value => Left(value.toString)
  }

  def getDocumentForTraversable(sequence: Traversable[_]) = {
    sequence.foldLeft(text(""))({ (document, element) =>
